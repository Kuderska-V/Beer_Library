//
//  EditPresenter.swift
//  Beer_Library
//
//  Created by Vitalina Nazaruk on 26.04.2023.
//  Copyright (c) 2023 ___ORGANIZATIONNAME___. All rights reserved.
//
//  This file was generated by the Clean Swift Xcode Templates so
//  you can apply clean architecture to your iOS and Mac projects,
//  see http://clean-swift.com
//

import UIKit

protocol EditPresentationLogic {
    func presentUserDetails(response: Edit.User.Response)
}

class EditPresenter: EditPresentationLogic {
    weak var viewController: EditDisplayLogic?
  
    // MARK: Present User Details
    
    func presentUserDetails(response: Edit.User.Response) {
        let viewModel = Edit.User.ViewModel(firstName: response.firstName.capitalized, lastName: response.lastName.capitalized)
        viewController?.displayUserDetails(viewModel: viewModel)
    }
}
